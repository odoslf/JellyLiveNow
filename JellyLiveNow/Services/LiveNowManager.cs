...
public class LiveNowManager : IHostedService, IDisposable
{
    ...
    /// <summary>
    /// Gets the active Live TV channel GUID.
    /// </summary>
    public Guid ActiveChannelId
    {
        get
        {
            lock (_lock)
    ...
    /// <summary>
    /// Gets the active Live TV channel name.
    /// </summary>
    ...
    /// <summary>
    /// Gets the current program title if available.
    /// </summary>
    public string ActiveProgramTitle
    {
        get
        {
            lock (_lock)
            {
                return _activeProgramTitle;
            }
        }
    }

    /// <summary>
    /// Gets the active channel overview.
    /// </summary>
    public string ActiveOverview
    {
        get
        {
            lock (_lock)
            {
                return _activeOverview;
            }
        }
    }

    /// <summary>
    /// Gets the active channel image URL if available.
    /// </summary>
    ...
    /// <summary>
    /// Evaluates current active sessions and updates the active Live TV channel state.
    /// </summary>
    ...
    /// <summary>
    /// Attempts to extract the real Live TV channel GUID from a BaseItem.
    /// Returns false if item is a Movie, Episode, Audio, or non-Live TV item.
    /// </summary>
    public static bool TryGetLiveTvChannelId(BaseItem item, out Guid channelId)
    {
        ...
        if (item.ChannelId != Guid.Empty && ns != null && ns.Contains("LiveTv"))
        {
    ...
    private void UpdateActiveChannel(Guid channelId, BaseItem? item)
    {
        lock (_lock)
        {
            ...
            _activeImageUrl = channelItem?.PrimaryImagePath ?? string.Empty;
        }
    }
    ...
    /// <summary>
    /// Checks if the current banner notification is dismissed for the specified user.
    /// </summary>
    public bool IsDismissedForUser(Guid userId)
    {
        return _dismissedUserIds.ContainsKey(userId);
    }

    /// <summary>
    /// Dismisses the current active channel notification for the specified user.
    /// </summary>
